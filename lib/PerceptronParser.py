...
def train(parser, sentences, nr_iter=3):
	parser.tagger.start_training(sentences)
	for itn in range(nr_iter):
		correct = 0; total = 0
		random.shuffle(sentences)
		for words, gold_tags, gold_heads, gold_label in sentences:
...
def read_conll(loc):
	for sent_str in open(loc).read().strip().split('\n\n'):
		...
		for i, (word, pos, head, label) in enumerate(lines):
			...
			heads.append(int(head) + 1 if head != '-1' else len(lines) + 1)  # ROOT is placed at end of sentence not 0?
			labels.append(label)
		pad_tokens(words); pad_tokens(tags)
...
def main(model_dir, train_loc, heldout_in, heldout_gold):
	...
	input_sents = list(read_pos(heldout_in))  # why are these in separate files...???
	...
	for (words, tags), (_, _, gold_heads, gold_labels) in zip(input_sents, gold_sents):
		...
		for i, w in list(enumerate(words))[1:-1]:
			if gold_labels[i] in ('P', 'punct'):  # ignores punctuation?
				continue
